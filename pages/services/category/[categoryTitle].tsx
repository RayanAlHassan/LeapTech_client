...
const CategoryDetailPage = () => {
  ...
  // Fetch category info (to get image)
  useEffect(() => {
    if (!categoryTitle) return;
      ...
      .then((res) => {
        const found = res.data.find(
          (c: Category) =>
            ...
            (categoryTitle as string).trim().toLowerCase()
        );
  ...
  <div className="d-flex flex-column-reverse flex-md-row align-items-center justify-content-between">
    ...
    {selectedService ? (
      <div className="col-12 col-md-6 text-section text-center text-md-start mb-4 mb-md-0">
        ...
        <p className="lead description">{selectedService.description}</p>
        ...
        <div className="mt-3">
          ...
          >
            {showQuotationForm ? "Close Quotation Form" : "Request a Quote"}
          </button>
        </div>
      </div>
    ) : (
      <div className="col-12 col-md-6 text-center text-md-start mb-4 mb-md-0">
        <h1 className="category-title">{categoryTitle}</h1>
        <p className="lead description">Select a service to see details.</p>
      </div>
    )}
    ...
    <div className="col-12 col-md-6 text-center mb-4 mb-md-0">
      {category ? (
        ...
        <p style={{ color: "white" }}>Loading category image...</p>
      )}
    </div>
  </div>
</main>
